...
const labels: Record<string, string> = { STRONG_FIT: "Strong Fit", POTENTIAL_FIT: "Potential Fit", REVIEW: "Review", LOW_FIT: "Low Fit", NOT_RESEARCHED: "Not Researched", DISQUALIFIED: "Disqualified", COMPLETED: "Completed", ACTIVE: "Active", PUBLISHED: "Published", APPROVED: "Approved", APPROVED_WITH_CHANGES: "Approved + edits", RUNNING: "Running", WAITING: "Waiting", PENDING: "Pending", FAILED: "Failed", REJECTED: "Rejected", CHANGES_REQUESTED: "Changes requested", DRAFT: "Draft", SENT: "Sent", NEW: "New", ACTIONED: "Actioned", NOT_CONNECTED: "Not connected", CONNECTED: "Connected", CONFIGURED: "Configured" };
export function StatusBadge({ status }: { status: string }) {
  const tone = /FAILED|REJECTED|DISQUALIFIED/.test(status) ? "danger" : /WAITING|PENDING|REVIEW|CHANGES/.test(status) ? "warning" : /COMPLETED|ACTIVE|APPROVED|CONNECTED|STRONG/.test(status) ? "success" : /RUNNING|CONFIGURED|POTENTIAL/.test(status) ? "info" : "neutral";
  const Icon = tone === "danger" ? XCircle : tone === "warning" ? Clock3 : tone === "success" ? CheckCircle2 : tone === "info" ? AlertCircle : MinusCircle;
  return <Badge tone={tone}><Icon size={13} aria-hidden />{labels[status] ?? status.replaceAll("_", " ")}</Badge>;
}
export function FitScoreBadge({ score, status }: { score: number | null; status: string }) { return <span className="fit-score"><strong>{score ?? "—"}</strong><StatusBadge status={status} /></span>; }
