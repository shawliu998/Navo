...
const accountSeed = [
  ["Rheinwerk Automation GmbH", "rheinwerk-demo.example", "Germany", "Automotive Components", "500–1000", 92, "STRONG_FIT"],
  ["AlpenPack Maschinenbau GmbH", "horizonpack-demo.example", "Austria", "Packaging", "1000–5000", 87, "STRONG_FIT"],
  ["Helvetic Electronics AG", "vistula-demo.example", "Switzerland", "Electronics Manufacturing", "500–1000", 83, "STRONG_FIT"],
  ["Delta Motion Works B.V.", "deltamotion-demo.example", "Netherlands", "Industrial Equipment", "200–500", 74, "POTENTIAL_FIT"],
  ["Morava Precision s.r.o.", "morava-demo.example", "Czech Republic", "Automotive Components", "100–200", 68, "POTENTIAL_FIT"],
  ["Summit Assembly LLC", "summitassembly-demo.example", "United States", "Industrial Equipment", "200–500", 63, "POTENTIAL_FIT"],
  ["Nordlicht Components AG", "nordlicht-demo.example", "Germany", "Electronics Manufacturing", "50–100", 55, "REVIEW"],
  ["Tirol Process Machines GmbH", "tulipprocess-demo.example", "Austria", "Packaging", "100–200", 46, "REVIEW"],
  ["Baltic Handcraft Sp. z o.o.", "baltichandcraft-demo.example", "Poland", "Consumer Goods", "20–50", 32, "LOW_FIT"],
  ["Alpine Services GmbH", "alpineservices-demo.example", "Germany", "Professional Services", "20–50", 18, "LOW_FIT"],
  ["Bohemia Factory Lab s.r.o.", "bohemialab-demo.example", "Czech Republic", "Industrial Equipment", "100–200", null, "NOT_RESEARCHED"],
  ["Prairie Robotics Inc.", "prairierobotics-demo.example", "United States", "Industrial Equipment", "200–500", null, "NOT_RESEARCHED"],
] as const;
...
await db.transaction(async (tx) => {
  ...
  const evidenceRows = Array.from({ length: 20 }, (_, index) => ({ id: id(300 + index), workspaceId: DEMO_WORKSPACE_ID, createdBy: DEMO_USER_ID, accountId: id(100 + (index % 10)), type: ["PRODUCT_PAGE", "CAREERS", "WEBSITE", "NEWS"][index % 4]!, title: ["Inline inspection capability", "Automation engineering hiring", "Manufacturing footprint", "Production expansion"][index % 4]!, summary: ["The product page describes automated inline quality inspection.", "The careers page lists controls and machine-vision engineering roles.", "The company page describes multi-line manufacturing operations.", "The company announcement describes a planned production expansion."][index % 4]!, quote: ["Inline inspection is used on the final assembly line.", "Hiring: Controls Engineer — machine vision experience preferred.", "Operations include three manufacturing halls.", "A new production cell is planned for Q4."][index % 4]!, sourceUrl: `https://${accountSeed[index % 10]![1]}/demo-source-${index + 1}`, pageTitle: ["Product overview", "Careers", "Company operations", "Company news"][index % 4]!, observedAt: daysAgo(index + 1), fetchedAt: daysAgo(Math.max(0, index)), confidence: ["0.94", "0.86", "0.91", "0.78"][index % 4]!, accessible: index !== 17, contentHash: `demo-hash-${index + 1}`, metadata: { synthetic: true, provider: "mock-research" } }));
  ...
  await tx.insert(signals).values(Array.from({ length: 10 }, (_, index) => ({ id: id(600 + index), workspaceId: DEMO_WORKSPACE_ID, createdBy: DEMO_USER_ID, accountId: id(100 + index), type: ["QUALITY_INSPECTION", "EXPANSION", "NEW_FACTORY", "NEW_PRODUCT", "TRADE_SHOW"][index % 5]!, summary: ["Inline inspection on final assembly", "Production capacity expansion", "New manufacturing site", "New automated product line", "Exhibiting at an industry event"][index % 5]!, rationale: index === 0 ? "The product page explicitly states that inline inspection is used on the final assembly line." : "The signal is linked to a source excerpt and can be reviewed before it affects outreach.", evidenceUrls: [`https://${accountSeed[index]![1]}/demo-source-${index + 1}`], confidence: ["0.94", "0.82", "0.91", "0.76", "0.80"][index % 5]!, detectedAt: daysAgo(index), status: index < 4 ? "ACTIONED" : "NEW", ownerName: index % 2 ? "陈薇" : "刘晓岚" })));
  ...
  const messageRows = Array.from({ length: 12 }, (_, index) => ({ id: id(1100 + index), workspaceId: DEMO_WORKSPACE_ID, createdBy: DEMO_USER_ID, conversationId: index < 5 ? id(1050 + index) : null, accountId: id(100 + index), contactId: index < 10 ? id(200 + index) : null, sequenceId: id(800), runId: id(900 + (index % 15)), direction: "OUTBOUND", channel: "EMAIL", providerMessageId: index < 8 ? `email-sink-out-${index + 1}` : null, subject: `A question about ${accountSeed[index]![0]} quality inspection`, body: index === 0 ? `Hi Anna,\n\nI saw that Rheinwerk uses inline inspection on its final assembly line and operates three manufacturing halls. Nova Automation helps industrial manufacturers evaluate vision inspection for defects, dimensions, and surface quality, with integration into existing line controls.\n\nWould a brief conversation next week be useful to compare the inspection requirements for one priority station—particularly camera interfaces, traceability, and changeover constraints?\n\nSources:\nhttps://rheinwerk-demo.example/demo-source-1\nhttps://rheinwerk-demo.example/demo-source-11\n\nBest,\nNova Automation` : `Hi ${index < 10 ? contactRows[index]!.name : "there"},\n\nI saw that your public company materials describe multi-line manufacturing operations. Would it be useful to compare how inline vision inspection could fit one priority station?\n\nSource: https://${accountSeed[index]![1]}/demo-source-${index + 1}\n\nBest,\nNova Automation`, originalSubject: `A question about quality inspection`, originalBody: "Original deterministic draft", status: index < 8 ? "SENT" : index < 10 ? "PENDING_APPROVAL" : "DRAFT", sentAt: index < 8 ? daysAgo(index + 1) : null, replyClassification: index < 5 ? ["POSITIVE", "QUESTION", "REFERRAL", "NOT_NOW", "UNSUBSCRIBE"][index] : null, evidenceIds: index === 0 ? [id(300), id(310)] : [id(300 + index)], claimsUsed: ["Compatible with common industrial camera interfaces."], idempotencyKey: `${DEMO_WORKSPACE_ID}:${id(810)}:${id(1200 + index)}:${id(820)}:1` }));
  ...
  await tx.insert(approvals).values(Array.from({ length: 8 }, (_, index) => ({ id: id(1400 + index), workspaceId: DEMO_WORKSPACE_ID, createdBy: DEMO_USER_ID, runId: id(900 + (index % 12)), nodeRunId: id(950 + ((index * 4 + 3) % 20)), accountId: id(100 + index), contactId: id(200 + index), messageId: id(1100 + index), status: index === 0 ? "APPROVED" : index < 4 ? "PENDING" : index < 6 ? "APPROVED" : index === 6 ? "CHANGES_REQUESTED" : "REJECTED", risk: index === 3 ? "MEDIUM" : "LOW", originalSubject: messageRows[index]!.subject, originalBody: messageRows[index]!.body, editedSubject: index === 5 ? `${messageRows[index]!.subject} — technical fit` : null, editedBody: index === 5 ? `${messageRows[index]!.body}\n\nTechnical note available on request.` : null, diff: index === 5 ? { added: "Technical note available on request." } : {}, reviewerName: index === 0 ? "刘晓岚" : index >= 4 ? "王静" : null, reviewedAt: index === 0 ? daysAgo(2) : index >= 4 ? daysAgo(index - 3) : null, reason: index === 0 ? "Evidence and recipient verified before EmailSink delivery." : index === 7 ? "Contact history requires manual review." : null, persona: contactRows[index]!.persona, country: accountSeed[index]![2] })));
  ...
  const missionRows = [
    ...
    { id: id(1903), name: "Qualify autumn trade show account list", type: "TRADE_SHOW_LIST_QUALIFICATION", objective: "Research and prioritize the autumn trade show list.", desiredOutcome: "Return a ranked account shortlist.", status: "DRAFT", operatingMode: "OBSERVE", playId: id(700), inputSource: "TRADE_SHOW_LIST", targetCount: 25, processedCount: 0, qualifiedCount: 0, pendingApprovalCount: 0, progress: 0, currentStep: "Plan ready for review", agentSummary: "Draft mission is ready for target and cost review.", maximumAccounts: 25, estimatedCostLimit: "1.25", actualCost: "0", startedAt: null, dueAt: daysAgo(-14), completedAt: null },
  ...
  const seededBatchPlan = {
    ...
    objective: "Research one trade show account and prepare a safe draft.",
  ...
  await tx.insert(agentEvents).values(eventTypes.map((type, index) => ({
    ...
    description: index === 8 ? "Confidence is 0.42 because only one accessible source supports this finding." : index === 9 ? "Mock Research timed out for one synthetic website fixture; no external service was called." : undefined,
