...
export function AgentCommandComposer({ accounts, initialCommand = "", compact = false, onCreated }: { accounts: AccountOption[]; initialCommand?: string; compact?: boolean; onCreated?: (missionId: string) => void }) {
  ...
  const createMission = async (status: "DRAFT" | "ACTIVE") => {
    ...
    try {
      const response = await fetch("/api/agent/commands/create-mission", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ command: previewData.objective, name: previewData.name, status, accountIds: [account.id], preview: { plan: previewData.plan, provider: previewData.provider, model: previewData.model, plannerMode: previewData.plannerMode, fallbackReason: previewData.fallbackReason } }) });
  ...
  return <section className={compact ? undefined : "card agent-command-composer"} aria-label="Give Navo a command" style={compact ? undefined : { padding: 20 }}>
    {!compact && <div className="card-header" style={{ marginBottom: 12 }}><div><h2>Give Navo an outcome</h2><span className="card-subtitle">Select one account, inspect the exact MissionPlan, then save a draft-only mission.</span></div></div>}
