...
type Preview={provider:string;model:string;plan:{name:string;objective:string;targetDescription:string;steps:Array<{id:string;type:string;title:string;description:string}>;stopConditions:string[];expectedOutputs:string[];assumptions:string[]}};
type FormState={
...
function buildInitialForm(plays:PlayOption[],accounts:AccountOption[],params:ReturnType<typeof useSearchParams>,preset?:MissionPreset):FormState{
  ...
  if(preset){
    return{
      ...
      testMode:true,maximumAccounts:preset.maximumAccounts,estimatedCostLimit:1,dueAt:"",stopConditions:preset.stopConditions,
    };
  }
  return{
    name:"",type:"TARGET_ACCOUNT_DISCOVERY",objective:params.get("objective")??"",desiredOutcome:"",inputSource:"DEMO_ACCOUNTS",
    ...
    estimatedCostLimit:1,dueAt:"",stopConditions:"Best account, draft, task and memory are persisted\nMaximum 20 iterations\nTwo consecutive steps fail",
  };
}
...
export function MissionWizard({plays,accounts,preset}:{plays:PlayOption[];accounts:AccountOption[];preset?:MissionPreset}){
  ...
  const canContinue=step===0?form.name.trim().length>=3&&form.objective.trim().length>=8:step===4?form.maximumAccounts>0&&form.estimatedCostLimit>=0:true;
  ...
  async function loadPreview(){
    ...
    try{
      const response=await fetch("/api/agent/commands/preview",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({command:objective})});
    ...
    <div className={styles.wizardGrid}>
      <main className={styles.wizardCard}>
        ...
        {step===5&&<><h2>Review and create</h2><p className={styles.wizardIntro}>Review the exact schema-validated autonomous plan that will be persisted and executed.</p>
          ...
          {preview&&<section className={styles.reviewSection}><span>AI MissionPlan</span><strong>{preview.provider} · {preview.model}</strong><p className="muted" style={{margin:"6px 0"}}>{preview.plan.targetDescription}</p>{preview.plan.steps.map((item,index)=><div className="list-row" key={item.id}><span><strong>{index+1}. {item.title}</strong><small className="muted" style={{display:"block"}}>{item.description}</small></span><Badge tone="neutral">{label(item.type)}</Badge></div>)}<p className="muted" style={{margin:"9px 0 0"}}>Expected: {preview.plan.expectedOutputs.join(" · ")}</p></section>}
