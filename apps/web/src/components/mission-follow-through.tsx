...
export function MissionCompletionBrief({ brief, missionId }: { brief: CompletionBrief; missionId: string }) {
  ...
  return <section className="card" aria-label="Agent Completion Brief" style={{ borderColor: brief.state === "failed" ? "var(--danger)" : "var(--accent)", marginBottom: 18 }}>
    ...
    <div className="page-actions" style={{ marginTop: 14, flexWrap: "wrap" }}>
      {brief.mode === "research" && brief.accountId && ["completed", "empty"].includes(brief.state) && <Link className="button button-secondary" href={`/app/missions/new?accountId=${encodeURIComponent(brief.accountId)}&objective=${encodeURIComponent(objective)}`}><RotateCcw size={14}/>Research again</Link>}
      {brief.mode === "research" && brief.state === "completed" && <button className="button button-primary" type="button" onClick={() => void createTask()} disabled={busy !== null}>{busy === "task" ? <Loader2 className="spin" size={14}/> : <ClipboardPlus size={14}/>}Create follow-up task</button>}
      {brief.mode === "reply" && brief.state === "completed" && <Link className="button button-primary" href={`/app/missions/${missionId}?tab=results`}><FilePenLine size={14}/>Review reply draft</Link>}
