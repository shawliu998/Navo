...
export async function POST(request: NextRequest, { params }: { params: Promise<{ messageId: string }> }) {
  ...
  if (!input.success) return apiError("VALIDATION_ERROR", "Invalid complaint event.", 422, input.error.flatten());
  const { messageId } = await params;
  const result = await processEmailSinkEvent(messageId, { eventType: "SPAM_COMPLAINT", ...input.data });
