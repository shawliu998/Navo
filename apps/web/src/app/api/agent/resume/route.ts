...
import { enqueueAgentTick, enqueueMissionContinuation } from "@/lib/mission-queue";
...
export async function POST() {
  ...
  await Promise.all([enqueueAgentTick(DEMO_WORKSPACE_ID), ...deferred.map((mission) => enqueueMissionContinuation({ workspaceId: DEMO_WORKSPACE_ID, missionId: mission.id }))]);
