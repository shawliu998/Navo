...
export async function POST() {
  ...
  const data = await setAgentPaused(DEMO_WORKSPACE_ID, DEMO_USER_ID, true);
  ...
  return NextResponse.json({ agent: { status: "PAUSED", headline: "Navo is paused", detail: data.currentActivity, activeMissions: snapshot.activeMissionCount, pendingApprovals: snapshot.pendingApprovalCount, operatingMode: data.operatingMode }, data });
}
