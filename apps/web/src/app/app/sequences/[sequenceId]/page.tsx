...
export default async function SequencePage({ params }: { params: Promise<{ sequenceId: string }> }) {
  ...
  return <div className="page">
    <Link href="/app/sequences" className="muted" style={{ display: "inline-flex", gap: 6, alignItems: "center", marginBottom: 12 }}><ArrowLeft size={14} />返回 Sequences</Link>
    <PageHeader eyebrow="SEQUENCE EDITOR" title={data.sequence.name} description="Europe/Berlin · 周一至周五 09:00–16:30 · 每日上限 25" actions={<><StatusBadge status={data.sequence.status} /><Badge tone="warning">TEST MODE</Badge><Button disabled title="Sequence publishing is not available in this Alpha.">Publish</Button></>} />
    <div className="alert alert-info" style={{ marginBottom: 16 }}><ShieldCheck size={16} /><span>真实发送关闭。Test Send 仅允许 demo@navo.local，所有消息都保留 TEST 标记。</span></div>
