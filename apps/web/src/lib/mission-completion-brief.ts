...
export function deriveCompletionBrief(input: BriefInput): CompletionBrief {
  ...
  if (input.status === "FAILED") return {
    state: "failed", conclusion: "Mission stopped before a complete brief could be delivered.", qualified: null, score: null,
  ...
  if (input.status !== "COMPLETED") return {
    state: "pending", conclusion: "Navo will publish a completion brief after persisted research and qualification are available.", qualified: null, score: null,
  ...
  const qualified = qualification ? !["LOW_FIT", "DISQUALIFIED"].includes(qualification.status) : null;
  const findings = [
  ...
  if (!qualification && !findings.length && !input.message) return {
    state: "empty", conclusion: "This completed mission has no persisted qualification, findings, or draft to summarize.", qualified: null, score: null,
  ...
  return {
    state: "completed",
    conclusion: qualified === true ? `${input.account?.name ?? "The account"} is qualified for a safe follow-up.` : qualified === false ? `${input.account?.name ?? "The account"} is not currently qualified for outreach.` : "Research was saved; qualification is unavailable.",
    qualified,
    ...
    recommendedNextStep: qualified === false ? "Review the evidence before pursuing this account." : "Review the DRAFT, then create a follow-up task if appropriate.",
